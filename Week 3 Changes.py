...
class TwoLevel:
    ...
    def to_unitary(self) -> np.ndarray:
        ...
        result[np.ix_([self.level0, self.level1], [self.level0, self.level1])] = self.unitary
        return result
...
class SingleQubitGate:
    ...
    def to_unitary(self) -> np.ndarray:
        ...
        for i in range(N):
            if not (i & (1 << self.qubit)):
                j = i ^ (1 << self.qubit)
                result[np.ix_([i, j], [i, j])] = self.unitary
...
class ControlledU:
    ...
    def to_unitary(self) -> np.ndarray:
        ...
        control_state = (1 << self.n) - 1 - (1 << self.target)
        state0 = control_state
        state1 = control_state ^ (1 << self.target)
        
        result[np.ix_([state0, state1], [state0, state1])] = self.unitary
        return result
...
def error_up_to_phase(a: np.ndarray, b: np.ndarray) -> float:
    ...
    if np.abs(overlap) < 1e-10:
        return np.linalg.norm(b - a, 'fro')
    phase = overlap / np.abs(overlap)
    aligned_b = phase * b
    return np.linalg.norm(aligned_b - a, 'fro')
...
def decompose_vector(vec: np.ndarray) -> TwoLevels:
    ...
    for i in range(N - 1, 0, -1):
        ...
        if norm > 1e-10:
            u = align(x, y, norm)
            two_levels.append(TwoLevel(size=N, level0=i - 1, level1=i, unitary=u))
            current_vec[i - 1] = norm
            current_vec[i] = 0
    
    return list(reversed(two_levels))
...
def two_levels_to_unitary(two_levels: TwoLevels) -> np.ndarray:
    ...
    """
    N = two_levels[0].size if two_levels else 1
    result = np.eye(N, dtype=complex)
...
def decompose_unitary(u: np.ndarray) -> TwoLevels:
    ...
    for k in range(N - 1):
        subcolumn = current_u[k:, k]
        sub_two_levels = decompose_vector(subcolumn)
        expanded = expand_twolevels(sub_two_levels, num_qubits(N))
        
        two_levels.extend(expanded)
        
        for tl in expanded:
            current_u = tl.to_unitary() @ current_u
    
    phase = np.angle(current_u[-1, -1])
    if abs(phase) > 1e-10:
        phase_mat = np.array([[np.exp(-1j * phase / 2), 0], [0, np.exp(1j * phase / 2)]], dtype=complex)
        two_levels.append(TwoLevel(size=N, level0=N - 2, level1=N - 1, unitary=phase_mat))
...
def abc_reconstruct(d: ABC) -> np.ndarray:
    ...
    X = np.array([[0, 1], [1, 0]], dtype=complex)
    phase = np.exp(1j * d.alpha)
    return phase * d.A @ X @ d.B @ X @ d.C
...
def gray_code(tl: TwoLevel) -> list[Swap]:
    ...
    for target in range(n):
        if ((code ^ tl.level1) >> target) & 1:
            control_vals = [bool((code >> i) & 1) for i in range(n)]
            swaps.append(Swap(target=target, control_vals=control_vals))
            code ^= 1 << target
...
def decompose_swap(swap: Swap) -> Circuit:
    ...
    return controlled_circuit(n, swap.target, swap.control_vals, 
...
def controlled_circuit(
    ...
    for q in range(n):
        if q != target and not control_vals[q]:
            circuit.append(SingleQubitGate(n=n, qubit=q, unitary=X))
...
def decompose_twolevel(tl: TwoLevel) -> Circuit:
    ...
    if swaps:
        last_swap = swaps[-1]
        circuit.extend(controlled_circuit(num_qubits(tl.size), last_swap.target, 
                                         last_swap.control_vals, tl.unitary))
...
def decompose_controlled(
    ...
    circuit = []
    circuit.extend(decompose_controlled(n, remaining_controls, target, V))
    
    X = np.array([[0, 1], [1, 0]], dtype=complex)
    circuit.extend(decompose_controlled(n, remaining_controls, pivot, X))
    
    V_dag = V.conj().T
    circuit.extend(decompose_controlled(n, remaining_controls, target, V_dag))
...
def decompose_cu(g: CU) -> Circuit:
    ...
    circuit = []
    circuit.append(SingleQubitGate(n=g.n, qubit=g.target, unitary=abc.C))
    circuit.append(CNOT(n=g.n, control=g.control, target=g.target))
    circuit.append(SingleQubitGate(n=g.n, qubit=g.target, unitary=abc.B))
    circuit.append(CNOT(n=g.n, control=g.control, target=g.target))
    circuit.append(SingleQubitGate(n=g.n, qubit=g.target, unitary=abc.A))
    
    phase_gate = np.array([[1, 0], [0, np.exp(1j * abc.alpha)]], dtype=complex)
    circuit.append(SingleQubitGate(n=g.n, qubit=g.control, unitary=phase_gate))
...
def decompose_to_basis(u: np.ndarray) -> Circuit:
    ...
    """
    n = num_qubits(u.shape[0])
    
    circuit = to_circuit(twolevel_decomposition(u))
    
    circuit = [
        gate for tl in circuit
        for gate in (decompose_twolevel(tl) if isinstance(tl, TwoLevel) else [tl])
    ]
    
    circuit = [
        gate for g in circuit
        for gate in (decompose_controlledU(g) if isinstance(g, ControlledU) else [g])
    ]
    
    circuit = [
        gate for g in circuit
        for gate in (decompose_cu(g) if isinstance(g, CU) else [g])
    ]
...
def ht_gates(n: int, qubit: int, word: str) -> Circuit:
    ...
    for char in reversed(word):
        if char == 'H':
            circuit.append(SingleQubitGate(n=n, qubit=qubit, unitary=H))
        elif char == 'T':
            circuit.append(SingleQubitGate(n=n, qubit=qubit, unitary=T))
...
def decompose_to_ht(u: np.ndarray, error: float) -> Circuit:
    ...
    for gate in circuit:
        if isinstance(gate, SingleQubitGate):
            ...
            result.extend(ht_gates(n, gate.qubit, word))
        else:
            result.append(gate)
